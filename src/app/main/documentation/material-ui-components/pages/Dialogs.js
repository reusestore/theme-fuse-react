...
<Typography className="mb-16" component="div">You may make a dialog responsively full screen using <code>{`withMobileDialog`}</code>. By default, <code>{`withMobileDialog()(Dialog)`}</code> responsively full screens <em>at or below</em> the <code>{`sm`}</code> <a href="/customization/breakpoints/">screen size</a>. You can choose your own breakpoint for example <code>{`xs`}</code> by passing the <code>{`breakpoint`}</code> argument: <code>{`withMobileDialog({breakpoint: &#39;xs&#39;})(Dialog)`}</code>.</Typography>
<Typography className="mb-16" component="div"><FuseExample
                    className="my-24"
                    iframe={false}
